...
namespace SharpAssembler.Architectures.X86.Tests.Opcodes
{
	...
	public class CMovLETests : OpcodeTestBase
	{
		...
		{
			var instruction = Instr.CMovLE(Register.DX, new EffectiveAddress(DataSize.Bit16, DataSize.None, c => new ReferenceOffset(0x36C5)));

			// CMOVLE dx, WORD [0x36C5]
			AssertInstruction(instruction, DataSize.Bit16, new byte[] { 0x0F, 0x4E, 0x16, 0xC5, 0x36 });
			AssertInstruction(instruction, DataSize.Bit32, new byte[] { 0x66, 0x0F, 0x4E, 0x15, 0xC5, 0x36, 0x00, 0x00 });
			AssertInstruction(instruction, DataSize.Bit64, new byte[] { 0x66, 0x0F, 0x4E, 0x14, 0x25, 0xC5, 0x36, 0x00, 0x00 });
		}
