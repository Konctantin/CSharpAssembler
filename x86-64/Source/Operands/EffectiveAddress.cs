...
namespace SharpAssembler.x86.Operands
{
		...
		IRegisterOrMemoryOperand, ISourceOperand
	{
		...
		/// or <see cref="SharpAssembler.Core.DataSize.None"/> to specify no particular address size.</value>
		/// <remarks>
		/// When set to a value other than <see cref="SharpAssembler.Core.DataSize.None"/>, the specified address
		/// size is used. The registers are checked to have the same size. When set to
		/// <see cref="SharpAssembler.Core.DataSize.None"/>, the address size is determined from the registers. When
		...
		/// absolute address; otherwise, <see langword="null"/> to use the <see cref="Architecture"/>'s default.
		...
		internal override bool IsMatch(Instruction.OperandDescriptor descriptor)
		{
			switch (descriptor.OperandType)
			{
				case Instruction.OperandType.RegisterOrMemoryOperand:
					return this.Size == descriptor.RegisterType.GetSize();
				case Instruction.OperandType.MemoryOperand:
		...
		/// <see cref="Instruction.OperandDescriptor"/>.
		/// </summary>
		/// <param name="descriptor">The <see cref="Instruction.OperandDescriptor"/> used to
		/// adjust.</param>
		/// <remarks>
		/// Only <see cref="Instruction.OperandDescriptor"/> instances for which <see cref="IsMatch"/>
		/// returns <see langword="true"/> may be used as a parameter to this method.
		/// </remarks>
		internal override void Adjust(Instruction.OperandDescriptor descriptor)
		{
			// Nothing to do.
		}
