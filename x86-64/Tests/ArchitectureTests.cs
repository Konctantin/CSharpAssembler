...
namespace SharpAssembler.x86.Tests
{
	...
	public class ArchitectureTests
	{
		...
		public void ConstructorTest_CpuType()
		{
			...
			var arch = new Architecture(type);
		...
		public void ConstructorTest_CpuFeatures()
		{
			...
			var arch = new Architecture(features);
		...
		public void ConstructorTest_CpuType_DataSize()
		{
			...
			var arch = new Architecture(type, size);
		...
		public void ConstructorTest_CpuFeatures_DataSize()
		{
			...
			var arch = new Architecture(features, size);
		...
		public void ConstructorTest_CpuType_CpuFeatures_DataSize()
		{
			...
			var arch = new Architecture(type, features, size);
		...
		public void NameTest()
		{
			...
			arch = new Architecture(CpuType.IntelNehalem);
			Assert.AreEqual("Intel codename Nehalem (x86-64)", arch.Name);

			arch = new Architecture(CpuFeatures.Privileged);
		...
		{
			var arch = new Architecture();
			var objectFile = new BinFormat.BinObjectFile("test", arch);
		...
		{
			Assert.IsTrue(Architecture.IsValidAddressSize(CpuType.AmdBulldozer, DataSize.Bit64));
			Assert.IsFalse(Architecture.IsValidAddressSize(CpuType.IntelPentium4, DataSize.Bit64));

			Assert.IsTrue(Architecture.IsValidAddressSize(null, DataSize.Bit64));
			Assert.IsFalse(Architecture.IsValidAddressSize(null, DataSize.Bit256));
		}
	}
}
